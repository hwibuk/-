...
# 사용자님이 작성한 RobotState 클래스와 인스턴스를 가져옵니다.
# 파일 구조가 models/robot_state.py 인 경우입니다.
try:
    from models.robot_state import robot_manager
except ImportError:
    # 만약 파일명이 다르다면 아래처럼 직접 클래스를 정의하거나 파일명을 확인하세요.
...
CORS(app)  # 브라우저에서 Flask 데이터에 접근할 수 있도록 허용합니다.
